...
const STEPS = ['Personal Info', 'Income & Expenses', 'Assets & Debts', 'Protection', 'Review'];
...
export default function PublicBalanceSheetPage({ params }: { params: Promise<{ agentCode: string }> }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.push(`/b/${agentCode}/results`);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-950 py-8 px-4">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          <div className="inline-block w-10 h-10 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-xl mb-3" />
          <h1 className="text-2xl font-bold text-white">Free Financial Analysis</h1>
          <p className="text-gray-400 text-sm mt-1">Get your personalized financial health report.</p>
        </div>
        ...
        <div className="flex gap-1 mb-8">
          {STEPS.map((s, i) => (
            <button key={s} onClick={() => i <= step && setStep(i)}
              className={`flex-1 h-1.5 rounded-full transition-colors ${i <= step ? 'bg-blue-500' : 'bg-gray-800'}`} title={s} />
          ))}
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 sm:p-8">
          ...
          {step === 0 && (
            <div className="space-y-4">
              <Row><Field label="First Name" value={formData.firstName} onChange={v => update('firstName', v)} /><Field label="Last Name" value={formData.lastName} onChange={v => update('lastName', v)} /></Row>
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  <CurrField label="Housing" value={formData.housing} onChange={v => update('housing', v)} />
                  <CurrField label="Utilities" value={formData.utilities} onChange={v => update('utilities', v)} />
                  <CurrField label="Food" value={formData.food} onChange={v => update('food', v)} />
                  <CurrField label="Transportation" value={formData.transportation} onChange={v => update('transportation', v)} />
                  <CurrField label="Insurance" value={formData.insurance} onChange={v => update('insurance', v)} />
                  ...
                  <CurrField label="Other" value={formData.otherExpenses} onChange={v => update('otherExpenses', v)} />
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <CurrField label="Brokerage" value={formData.brokerageAccounts} onChange={v => update('brokerageAccounts', v)} />
              ...
              <div>
                ...
                <div className="space-y-3">
                  <CurrField label="Mortgage" value={formData.mortgage} onChange={v => update('mortgage', v)} />
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div>
                <h3 className="text-sm font-medium text-gray-400 mb-3">Insurance</h3>
                <div className="space-y-3">
                  <CurrField label="Life Insurance" value={formData.lifeInsuranceCoverage} onChange={v => update('lifeInsuranceCoverage', v)} />
                  <CurrField label="Disability (monthly)" value={formData.disabilityMonthlyBenefit} onChange={v => update('disabilityMonthlyBenefit', v)} />
                </div>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-400 mb-3">Estate Planning</h3>
                <div className="space-y-3">
                  <Toggle label="Will" value={formData.hasWill} onChange={v => update('hasWill', v)} />
                  <Toggle label="Trust" value={formData.hasTrust} onChange={v => update('hasTrust', v)} />
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-gray-800">
            ...
            {step < STEPS.length - 1 ? (
              ...
              <button onClick={handleSubmit} disabled={saving}
                ...
                <CheckCircle className="w-4 h-4" /> {saving ? 'Generating...' : 'See My Results'}
              </button>
            )}
          </div>
        </div>
